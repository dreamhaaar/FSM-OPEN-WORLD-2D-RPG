...
func _ready():
	...
	# start after the whole scene tree is ready
	call_deferred("_start_hfsm_safely")

	#  debounce timer for "Player Lost"
...
func go_hurt():
	...
	var current_name = hfsm.current_state.name
	
	# DFA Logic: Current State + Input(Damage) = Specific Next State
	if current_name == "Passive":
		hfsm.transition_to("P_Hurt", "DamageTaken")
		
	elif current_name == "Aggressive":
		hfsm.transition_to("A_Hurt", "DamageTaken")
		
	elif current_name == "Scared":
		hfsm.transition_to("S_Hurt", "DamageTaken")
